...
Generate a focused `CLAUDE.md` inside a target folder, plus an `AGENTS.md` alongside it. The file gives any future agent (Claude Code, Codex, etc.) the folder-specific context the global `CLAUDE.md` doesn't cover.

**Windows note:** symlinks require Developer Mode or an elevated shell. Prefer a
one-line `AGENTS.md` containing `See CLAUDE.md in this folder.` over a symlink —
it survives `git clone` on every platform, which a symlink does not.

## Process

### Step 1: Confirm the target folder + sanity-check it deserves a file
Ask the user which folder, and use its absolute path.
