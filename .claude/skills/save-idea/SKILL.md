...
3. **Read the target file** and find the last entry number. Next number = last + 1. Both files start at 1 if new. Never renumber anything.
...
- If `~/content/` or either file is missing, create it with a one-line header (topics + podcast material; video ideas live in VIDEO-IDEAS.md), then append entry 1.
